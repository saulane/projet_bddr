        ...
        with open(f"{DATA_DIR}/{d}/{f}","r",encoding="utf8") as f_in:
            ...
            for row in reader:
                ...
                try:
                    ...
                    continue
                link = row["Study Link"].lstrip("https://doi.org/")
                theme = d[2:].split("/")[-1]
                study_type = row["Study Type"] if "Study Type" in row else None
                if len(dict_name_theme) > 0 and dict_name_theme[-1]["Study"] != study:
                    dict_name_theme.append({ 'Study': study,'Theme': theme,'Subtheme':  f.rstrip("csv"),"StudyType": study_type,"Date":date})
...
with open("test.csv", 'w', encoding="utf8",newline='') as csvfile:
    writer = csv.DictWriter(csvfile, fieldnames=["Study", "Theme", "Subtheme","StudyType","Date"], delimiter=";")
    writer.writeheader()
